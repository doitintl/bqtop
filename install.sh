...
PROJECTID=`firebase list | grep -i -w $1 | awk 'BEGIN { FS="│" } { printf $3 }' | sed 's/ //g'`
if [ "$1" != "$PROJECTID" ]; then
...
gcloud logging sinks create bqtop-running-jobs-export pubsub.googleapis.com/projects/$PROJECTID/topics/bqtop-running-jobs --project=$PROJECTID --log-filter 'resource.type="bigquery_resource" protoPayload.methodName="jobservice.insert" protoPayload.serviceData.jobInsertRequest.resource.jobConfiguration.dryRun!=true protoPayload.serviceData.jobInsertRequest.resource.jobConfiguration.query:*' --quiet >/dev/null || error_exit "Error creating Log Export sink"
